...
class EncP(nn.Module):  
    def __init__(self, in_channels, input_points, num_stages, embed_dim, k_neighbors, alpha, beta, AggregationGPE_block, dim_expansion, type):
        super().__init__()
        ...
        for i in range(self.num_stages):
            ...
            self.AggregationGPE_list.append(AggregationGPE(out_dim, self.alpha, self.beta, AggregationGPE_block[i], dim_expansion[i], type))
...
class Point_PN_mn40(nn.Module):
    def __init__(self, in_channels=3, class_num=40, input_points=1024, num_stages=4, embed_dim=36, k_neighbors=40, beta=100, alpha=1000, AggregationGPE_block=[2,1,1,1], dim_expansion=[2,2,2,1], type='mn40'):
        super().__init__()
        # Parametric Encoder
        self.EncP = EncP(in_channels, input_points, num_stages, embed_dim, k_neighbors, alpha, beta, AggregationGPE_block, dim_expansion, type)
...
class Point_PN_scan(nn.Module):
    def __init__(self, in_channels=4, class_num=15, input_points=1024, num_stages=4, embed_dim=36, k_neighbors=40, beta=100, alpha=1000, AggregationGPE_block=[2,1,1,1], dim_expansion=[2,2,2,1], type='scan'):
        super().__init__()
        # Parametric Encoder
        self.EncP = EncP(in_channels, input_points, num_stages, embed_dim, k_neighbors, alpha, beta, AggregationGPE_block, dim_expansion, type)
        self.out_channel = embed_dim
