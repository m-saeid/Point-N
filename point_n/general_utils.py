...
def setup_parser():
    ...
    # Fix: Ensure these are present
    parser.add_argument("--init_dim", type=int, default=72, help="Initial embedding dimension")  # ✅ Add this
    parser.add_argument("--stages", type=int, default=4, help="Number of stages")
    parser.add_argument("--stage_dim", type=int, default=72, help="Dimension after each stage (feature dim)")  # ✅ Add this
    ...
    parser.add_argument("--embedding_fn", type=str, default="gpe", 
                        help="Embedding function to use: gpe, sine, cosine, tanh, poly, lap, recip, sinc, etc.")
    ...
    # Additional parameters for enhanced embeddings
    parser.add_argument("--blend", type=float, default=0.5, help="Blend value for hybrid embeddings")
    parser.add_argument("--fusion_method", type=str, default="variance", choices=["variance", "curvature"],
                        help="Fusion method used in multi-hybrid and adaptive fusion embeddings")
    parser.add_argument("--eps", type=float, default=1e-6, help="Epsilon value for numerical stability")
    parser.add_argument("--base_sigma", type=float, default=0.3, help="Base sigma for multi-hybrid embeddings")
    parser.add_argument("--blend_strategy", type=str, default="variance", help="Blend strategy for hybrid embeddings")
    parser.add_argument("--cues", type=str, default="curvature", choices=["curvature", "laplacian"],
                        help="Additional cues used in enriched geometric embedding")
...
def check_if_combination_exists(data, params):
    for entry in data:
        if (
            entry["sigma"] == params.sigma and
            entry["blend"] == params.blend and
            entry["fusion_method"] == params.fusion_method and
            entry["eps"] == params.eps and
            entry["cues"] == params.cues and
            entry["seed"] == params.seed and
            entry["init_dim"] == params.init_dim and
            entry["stage_dim"] == params.stage_dim and
            entry["stages"] == params.stages and
            entry["k"] == params.k
        ):
            return True
    return False
...
'''
def add_new_entry(data, batch, combo, acc_1nn, acc_cos, gamma, train_time, test_time):
    # Now combo should be a tuple containing:
    # (stage, sigma, seed, init_dim, stage_dim, k, blend, fusion_method, eps, base_sigma, blend_strategy, cues)
    stage, sigma, seed, init_dim, stage_dim, k, blend, fusion_method, eps, base_sigma, blend_strategy, cues = combo
    new_entry = {
        "batch": batch,
        "sigma": sigma,
        "blend": blend,
